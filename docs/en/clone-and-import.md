...
`git clone https://github.com/ByteLegendQuest/java-double-comparison`

A project directory named `java-double-comparison` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-double-comparison` directory which is created just now.
